...
const ProjectDetail = () => {
  ...
  // Admin can always manage; Manager can manage only if they are a member
  const canManage =
    user?.role === ROLES.ADMIN ||
    project?.createdBy?._id === user?.id;

  // A user can create tasks if they are Admin, or if they are a Manager
  // who is a member of this project
  const isMemberOfProject = project?.members?.some(
    (m) => (m._id || m) === user?.id
  );
  const canCreateTask =
    user?.role === ROLES.ADMIN ||
    (user?.role === ROLES.MANAGER && isMemberOfProject);
    ...
    <div className="project-detail">
      ...
      <div className="detail-info-grid">
        <div className="detail-info-card">
          ...
          <span className="detail-info-value">{project.createdBy?.name}</span>
        </div>
        {project.deadline && (
          <div className="detail-info-card">
